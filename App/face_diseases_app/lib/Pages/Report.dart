...
itemBuilder: (context, index) {
  ...
  var formattedDate = DateFormat('yyyy-MM-dd – kk:mm').format(DateTime.parse(dateStr));
...
return Card(
    ...
    margin: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), // Adjust the margin
    child: ListTile(
      title: Text(formattedDate, style: TextStyle(color: Colors.blue)),
      subtitle: Padding(
        padding: const EdgeInsets.all(8.0), // Add padding inside the card
        child: Text(results, style: TextStyle(fontSize: 16)), // Style subtitle text
      ),
      isThreeLine: true,
      contentPadding: const EdgeInsets.all(10), // Padding inside ListTile
    ),
  );
},
